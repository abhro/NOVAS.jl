...
function read_iau2000a()
    ...
    cls = lunisolar[1:678, [7, 8, 11, 9, 10, 13]] .* 1.0e4  # [:A, :A′, :A″, :B, :B′, :B″]
    napl = planetary[1:687, 2:15]                    # [:l, :l′, :F, :D, :Ω, :Me, :Ve, :E, :Ma, :J, :Sa, :U, :Ne, :pA]
    cpl = planetary[1:687, 17:20] .* 1.0e4           # [:A, :A″, :B, :B″]
    return nals, cls, napl, cpl
end
...
function read_cterms()
    ...
    ke1 = cterms[34, 4:17]
    se0 = cterms[1:33, 2:3] .* 1.0e-6
    se1 = cterms[34, 2:3] .* 1.0e-6
...
"""
norm_ang(angle::Real) = rem2pi(angle, RoundDown)
...
function ee_ct(jd_high::T, jd_low::T; accuracy::Symbol = :full) where {T <: Real}
    ...
    if accuracy == :full
        ...
        fa[1] = norm_ang(
            (
                485868.249036 +
                    (715923.2178 + (31.8792 + (0.051635 + (-0.0002447) * t) * t) * t) *
                    t
            ) * ASEC2RAD + rem(1325.0 * t, 1.0) * 2π
        )
        ...
        fa[2] = norm_ang(
            (
                1287104.793048 +
                    (
                    1292581.0481 +
                        (-0.5532 + (+0.000136 + (-0.00001149) * t) * t) * t
                ) * t
            ) *
        ...
        fa[3] = norm_ang(
            (
                335779.526232 +
                    (
                    295262.8478 +
                        (-12.7512 + (-0.001037 + (0.00000417) * t) * t) * t
                ) * t
            ) *
        ...
        fa[4] = norm_ang(
            (
                1072260.703692 +
                    (
                    1105601.209 +
                        (-6.3706 + (0.006593 + (-0.00003169) * t) * t) * t
                ) * t
            ) *
